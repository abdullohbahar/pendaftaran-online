                            <div class="row mt-4 mt-2">
                                <div class="col-12">
                                    <b>Alamat Pemilik</b>
                                </div>
                                <div class="col-sm-12 col-md-4 col-lg-3 mt-2">
                                    <label>Provinsi</label>
                                    <select name="provinsi_pemilik" class="select2-data-array browser-default"
                                        id="provinsiPemilik">
                                    </select>
                                </div>
                                <div class="col-sm-12 col-md-4 col-lg-3 mt-2">
                                    <label>Kabupaten</label>
                                    <select name="kabupaten_pemilik" class="select2-data-array browser-default"
                                        id="kabupatenPemilik">
                                    </select>
                                </div>
                                <div class="col-sm-12 col-md-4 col-lg-3 mt-2">
                                    <label>Kecamatan</label>
                                    <select name="kecamatan_pemilik" class="select2-data-array browser-default"
                                        id="kecamatanPemilik">
                                    </select>
                                </div>
                                <div class="col-sm-12 col-md-4 col-lg-3 mt-2">
                                    <label>Kelurahan</label>
                                    <select name="kelurahan_pemilik" class="select2-data-array browser-default"
                                        id="kelurahanPemilik">
                                    </select>
                                </div>

                                <input type="hidden" name="provinsi_pemilik" id="hiddenProvPemilik">
                                <input type="hidden" name="kabupaten_pemilik" id="hiddenKabPemilik">
                                <input type="hidden" name="kecamatan_pemilik" id="hiddenKecPemilik">
                                <input type="hidden" name="kelurahan_pemilik" id="hiddenKelPemilik">

                            </div>
